...
<div class="products-keywork">   
    <%
        if (ketQua != null && !ketQua.isEmpty()) {
            for (SanPham sp : ketQua) {
    %>
        <div class="product-keywork">
            <img src="<%=request.getContextPath() + "/images/" + sp.getDuongDanAnh()%>" alt="<%=sp.getTenSanPham()%>" />
            <h3><%=sp.getTenSanPham()%></h3>
        	<p>Giá: <%=String.format("%,.0f", sp.getGia())%> VNĐ</p>
        	<a href="<%=request.getContextPath()%>/productDetail?maSanPham=<%= sp.getMaSanPham() %>"    class="btn">    Xem chi tiết</a>
        </div>
    <%
            }
        } else {
